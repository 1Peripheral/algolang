...
void Token::set(TokenKind kind) { this->kind = kind; }

void Token::set(std::string lexeme) { this->lexeme = lexeme; }
...
TokenKind Token::checkIfKeyword(std::string str) {
  Token token;
  for (auto iter = kindStringRepr.begin(); iter != kindStringRepr.end();
       iter++) {
    if (iter->second == str)
      return iter->first;
  }
